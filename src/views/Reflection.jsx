...
// -----------------------------------------------------------------------------
// 🔮 1. CARD MEANING DICTIONARIES (Smart Logic)
// -----------------------------------------------------------------------------

// Definitions for the 22 Major Arcana cards
const MAJOR_ARCANA = {
...
// Helper function to generate meanings for the 56 Minor Arcana cards
const getMinorArcanaMeaning = (cardName) => {
  // Define the energy of each suit
  const suits = {
  ...
  // Define the energy of each number/rank
  const ranks = {
  ...
  // Check if the card name contains any Suit or Rank keywords
...
const Reflection = ({ 
  ...
  const [aiState, setAiState] = useState('idle'); // idle, loading, streaming, complete
  ...
  const handleChannelWisdom = () => {
    ...
    // 1. Determine the Wisdom based on Card Type
    let specificWisdom = "";
    
    // Check if it's in our Major Arcana list
    if (MAJOR_ARCANA[selectedCard.name]) {
        specificWisdom = MAJOR_ARCANA[selectedCard.name];
    } else {
        // Otherwise, generate it using the Minor Arcana Logic
        specificWisdom = getMinorArcanaMeaning(selectedCard.name);
    }

    // 2. Build the full message
    const fullMessage = `${selectedCard.name} has appeared. ${specificWisdom} In this moment, breathe deeply and accept this guidance.`;

    // 3. Simulate "Streaming" text (Typewriter effect)
    setTimeout(() => {
          ...
          <div className="space-y-4">
            {rituals.map((r) => (
              <div 
                ...
                style={{ 
                    backgroundColor: checkedItems[r] ? 'rgba(212, 175, 55, 0.1)' : 'rgba(255,255,255,0.05)'
                }}
              >
      ...
      <section className="relative z-10 max-w-6xl mx-auto mb-16 px-6 md:px-14">
        <div className="flex justify-between items-end mb-8 border-b border-white/10 pb-4">
            ...
            <button 
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all ${isGuided ? 'bg-white text-slate-900 shadow-[0_0_20px_rgba(255,255,255,0.3)]' : 'bg-white/10 text-white hover:bg-white/20'}`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
            ...
            <div key={id} className="flex flex-col gap-2 group">
                <div className="flex flex-col">
                    <h5 className="text-[11px] font-black tracking-[0.2em] text-gold uppercase" style={{ color: THEME.primary }}>
                        {isGuided ? PROMPTS[id].guidedLabel : PROMPTS[id].label}
                    </h5>
                    <span className="text-[10px] text-white/40 italic mt-1 min-h-[20px]">
                        {PROMPTS[id].sub}
                    </span>
                </div>
