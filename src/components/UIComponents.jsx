...
export const StatusHeader = ({ isOnline, onBack, className = "" }) => (
  <div className={`flex items-center gap-4 z-20 ${className}`}>
    {onBack && (
      <button 
        onClick={onBack} 
        className="w-8 h-8 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-white hover:bg-white/20 transition-all backdrop-blur-md hover:scale-105 active:scale-95"
      >
        <ChevronLeft size={16} />
      </button>
    )}
    <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-bold border transition-all ${isOnline ? 'border-green-400 text-green-300 bg-green-500/10' : 'border-orange-500/30 text-orange-400 bg-orange-500/5'}`}>
      <Cloud size={12} /> <span className="hidden xs:inline">{isOnline ? 'Live' : 'Offline'}</span>
    </div>
    <div className="flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-bold border border-white/20 text-white/60 bg-white/5">
...
export const BottomNav = ({ view, setView }) => (
  // FIX: Added 'bg-white' and specific shadow. Ensure text is visible.
  <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-6 pt-3 pb-8 md:pb-12 flex justify-around items-center z-50 rounded-t-[32px] md:rounded-t-[48px] shadow-[0_-20px_50px_rgba(0,0,0,0.15)] transition-all">
    <button onClick={() => setView('dashboard')} className={`flex flex-col items-center gap-1 transition-all ${['dashboard', 'reflection', 'tracker'].includes(view) ? 'text-slate-900 scale-110' : 'text-gray-400 hover:text-gray-600'}`}>
    ...
    <button onClick={() => setView('vault')} className={`flex flex-col items-center gap-1 transition-all ${view === 'vault' ? 'text-slate-900 scale-110' : 'text-gray-400 hover:text-gray-600'}`}>
