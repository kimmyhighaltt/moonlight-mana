...
export const Logo = ({ size = "text-xl", subtitle = "DAILY TAROT REFLECTION", showStars = true, color = THEME.primary }) => (
  <div className="flex flex-col items-center select-none text-center relative z-10 animate-fade-in">
    {showStars && (
...
export const StatusHeader = ({ isOnline, onBack }) => (
  <div className="absolute top-8 left-10 flex items-center gap-4 z-20">
    ...
    <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-bold border transition-all ${isOnline ? 'border-green-500/30 text-green-400 bg-green-500/5' : 'border-orange-500/30 text-orange-400 bg-orange-500/5'}`}>
      <Cloud size={12} /> {isOnline ? 'Live Ritual' : 'Offline Mode'}
    </div>
    <div className="flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-bold border border-white/10 text-white/40 bg-white/5">
      <ShieldCheck size={12} /> Privacy Locked
    </div>
  </div>
);

export const BottomNav = ({ view, setView }) => (
  <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-10 pt-4 pb-12 flex justify-between items-center z-50 rounded-t-[48px] shadow-[0_-20px_50px_rgba(0,0,0,0.15)] transition-all">
    <button onClick={() => setView('dashboard')} className={`flex flex-col items-center gap-1.5 transition-all ${['dashboard', 'reflection', 'tracker'].includes(view) ? 'text-slate-800 scale-110' : 'text-gray-300 hover:text-gray-400'}`}>
      <Home size={26} fill={['dashboard', 'reflection', 'tracker'].includes(view) ? 'currentColor' : 'none'} />
      <span className="text-[8px] font-black uppercase tracking-widest">Today</span>
    </button>
    <button onClick={() => setView('vault')} className={`flex flex-col items-center gap-1.5 transition-all ${view === 'vault' ? 'text-slate-800 scale-110' : 'text-gray-300 hover:text-gray-400'}`}>
      <BookOpen size={26} fill={view === 'vault' ? 'currentColor' : 'none'} />
      <span className="text-[8px] font-black uppercase tracking-widest">The Vault</span>
    </button>
    <button onClick={() => setView('planner')} className={`flex flex-col items-center gap-1.5 transition-all ${view === 'planner' ? 'text-slate-800 scale-110' : 'text-gray-300 hover:text-gray-400'}`}>
      <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-all ${view === 'planner' ? 'bg-gold shadow-lg shadow-gold/30' : 'bg-gray-100'}`} style={{ backgroundColor: view === 'planner' ? THEME.primary : '' }}>
        <Moon size={16} fill="white" color="white" />
      </div>
      <span className={`text-[8px] font-black uppercase tracking-widest ${view === 'planner' ? 'text-slate-800' : 'text-gray-300'}`}>Lunar</span>
    </button>
  </nav>
);